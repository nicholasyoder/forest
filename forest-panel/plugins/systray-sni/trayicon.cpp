...
void ti_cleanup_xcb_image(void *data) {
...
QImage trayicon::getImageNonComposite() const
{
    ...
    if (isTransparentImage(naiveConversion)) {
        ...
        return QImage(image->data, image->width, image->height, image->stride, QImage::Format_ARGB32, ti_cleanup_xcb_image, image);
    }
}
...
QImage trayicon::convertFromNative(xcb_image_t *xcbImage) const
{
    ...
    QImage image(xcbImage->data, xcbImage->width, xcbImage->height, xcbImage->stride, format, ti_cleanup_xcb_image, xcbImage);
...
void trayicon::enterEvent(QEvent *){
    ...
    int x = QCursor::pos().x();
    int y = QCursor::pos().y();
    QPoint local_pos = mapFromGlobal(QPoint(x,y));

    //stackContainerWindow(XCB_STACK_MODE_ABOVE);

    xcb_enter_notify_event_t* event = new xcb_enter_notify_event_t;
    memset(event, 0x00, sizeof (xcb_enter_notify_event_t));
    ...
    event->event_y = static_cast<int16_t>(local_pos.y());
    event->child = 0;
    event->state = 0;
    event->detail = 0;

    xcb_send_event(c, false, iWindowId, XCB_EVENT_MASK_ENTER_WINDOW, (char *) event);
    delete event;
...
void trayicon::leaveEvent(QEvent *){
    ...
    xcb_leave_notify_event_t* event = new xcb_enter_notify_event_t;
    memset(event, 0x00, sizeof (xcb_enter_notify_event_t));
    ...
    xcb_send_event(c, false, iWindowId, XCB_EVENT_MASK_LEAVE_WINDOW, (char *) event);
    delete event;
...
void trayicon::mouseMoveEvent(QMouseEvent *event){
    ...
    xcb_motion_notify_event_t* x_event = new xcb_motion_notify_event_t;
    memset(x_event, 0x00, sizeof (xcb_motion_notify_event_t));
    x_event->response_type = XCB_MOTION_NOTIFY;
    x_event->event = iWindowId;
    x_event->time = XCB_CURRENT_TIME;
    x_event->same_screen = 1;
    x_event->root = QX11Info::appRootWindow();
    x_event->root_x = x;
    x_event->root_y = y;
    x_event->event_x = static_cast<int16_t>(event->x());
    x_event->event_y = static_cast<int16_t>(event->y());
    x_event->child = 0;
    x_event->state = 0;
    x_event->detail = 0;

    xcb_send_event(c, false, iWindowId, XCB_EVENT_MASK_POINTER_MOTION, (char *) x_event);
    delete x_event;
