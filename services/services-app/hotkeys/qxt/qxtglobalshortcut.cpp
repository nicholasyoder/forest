...
bool QxtGlobalShortcutPrivate::activateShortcut(quint32 nativeKey, quint32 nativeMods)
{
          ...
          {
              qDebug() << "activate" << nativeKey;
                    emit shortcut->activated(QKeySequence( shortcut->shortcut() ));
...
void QxtGlobalShortcut::setactivated()
{
         emit activated();
}
...
void QxtGlobalShortcut::calleventFilter(xcb_generic_event_t *event)
{
    qxt_d().eventFilter(event);
}
