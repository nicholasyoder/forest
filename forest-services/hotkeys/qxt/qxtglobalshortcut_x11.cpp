...
void QxtGlobalShortcutPrivate::eventFilter(xcb_generic_event_t *event)
{
    //XEvent* oldevent = static_cast<XEvent*>(event);
    if (event->response_type == XCB_KEY_PRESS)
    {
        ...
        qDebug() << "key: " << key->detail;

        activateShortcut(key->detail, key->state & (ShiftMask | ControlMask | Mod1Mask | Mod4Mask));// Mod1Mask == Alt, Mod4Mask == Meta
    }
}
